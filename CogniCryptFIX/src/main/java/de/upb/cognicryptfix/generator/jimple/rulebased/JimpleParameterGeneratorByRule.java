...
public class JimpleParameterGeneratorByRule {
	...
	private Map<Local, List<Unit>> generateParameterUnit(CrySLVariable variable) {
		...
		if (variableType instanceof PrimType || variableType == Scene.v().getType("java.lang.String")) {
			...
			if (variableValue != null) {
				generatedUnits.putAll(arrayGenerator.generateArrayUnits(arrayType, Lists.newArrayList(variable.getValue())));
			} else {
			...
			if (!predicates.isEmpty()) {
				...
				Map<Local, List<Unit>> generatedParameterUnits = parameterGenerator.generateParameterUnits(initMethod, null, null);
